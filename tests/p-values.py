...
def CAM(corr_yx, corr_xy = 0.2, timesteps = 1000, dt = 1, noise = 0.3):
    x = np.array([0.5])
    y = np.array([0.5])
    t = np.arange(0, timesteps, dt)
    for i in range(1, timesteps):
...
def method_TE_kraskov_DCE(x, y, **kwargs):
    ...
    for column in results.columns.levels[0]:
        return results[column]['proc-0']['proc-1']
...
def obtain_pvalues(t, x, y, causal_method, nshuff = 100,  **kwargs): # arguments for the causal method
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=24) as executor:
        for i in range(nshuff):
            ...
            x3 = x1
            futures[i] = executor.submit(compute_pvalues_parallel, x1, x2, x3, y, causal_method, **kwargs)
...
if __name__ == '__main__':
    ...
    for system in systems:
        for method in methods:
            t,x,y = systems[system](corr_yx = 0.4, timesteps = 40, dt = 1, noise = 0.01)
