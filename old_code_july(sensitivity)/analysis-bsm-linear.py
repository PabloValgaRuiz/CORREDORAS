...
#____________________________________________________ GAM ____________________________________________________
    ...
    if 1: # DO NOT DO GAM IF IT ALREADY EXISTS
        ...
        for spec in myspecies:
            ...
            XX = np.linspace(mydf.index.min(), mydf.index.max(), n_splines)

            YY = np.interp(XX, species[spec]['prev_x'], species[spec]['prev_y']) # linear interpolation as a first step

            species[spec]['x'] = cp.copy(XX)
            species[spec]['y'] = cp.copy(YY)

        species_df = pd.DataFrame(species)
        species_df.to_pickle('linear_species_basa/species_%s.pkl' %(fit_type))
...
vx = delta13['age (kyr BP)'].astype('float')
vy = delta13['d13C (permil)']

YY = np.interp(XX, vx, vy) 
...
d13gam_df.to_pickle('linear_species_basa/d13gam_%s.pkl' %(fit_type))
...
vx = multipliers.index.astype('float')
...
gam_multipliers_df.to_pickle('linear_species_basa/gam_multipliers_%s.pkl' %(fit_type))