...
def main():
    ...
    out_base = f'{results_dir}/nullmodel/cond2'
    ...
    for j in range(0,200):
        dev_j_dir = f'{out_base}/network_j{j}'
        os.makedirs(dev_j_dir, exist_ok=True)
        os.makedirs(f'{dev_j_dir}/bootstrap_tables', exist_ok=True)

        # CREATE THE RANDOMIZED DATA. only the array changes, conditionals and multipliers stay the same
        randomized_array = np.zeros(array.shape)
        for i, spec in enumerate(myspecies):
            bootstrapped_indices = stationary_bootstrap(end-start)
            randomized_array[i] = array[i][bootstrapped_indices]
            
            # USE PAR INSTEAD OF COUNT
            if (PAR == True):
                randomized_array[i] = randomized_array[i] * conditional_variables['multipliers']
        
        # save the randomized data to a dataframe
        df_randomized = pd.DataFrame(randomized_array.T, columns=myspecies)
        pd.to_pickle(df_randomized, f'{dev_j_dir}/randomized_data_{start}-{end}.pkl')
        ...
        with ProcessPoolExecutor(max_workers=None) as executor:
            ...
            for b in executor.map(func, n_bootstrap):
                print(f"Completed bootstrap {b} for network j {j}")
