...
def compute_GAM_species(mydf, myspecies, n_splines, lam):
    fit_type = f'fit_{n_splines}_{lam}'
...
def compute_GAM_basa(mydf, myspecies, n_splines, lam = 0.01):

    fit_type = f'fit_{n_splines}_{lam}'

    XX, YY = compute_GAM_species(mydf = mydf, myspecies = myspecies, n_splines = n_splines, lam = lam)
    # ___________________________________ GAM over the species ___________________________________
...
def clean_basa_pollen_concentration(mydf_pollen, myspecies):
    ...
    pd.to_pickle(multipliers, 'data_temp/multipliers_basa.pkl')

    # remove species that have all zeros
...
#____________________________________________________ GAM ____________________________________________________
    ...
    n_splines = 140
    compute_GAM_basa(mydf = mydf, myspecies = myspecies, n_splines = n_splines, lam = 0.01)

# plot

plt.subplots(figsize=(10,6))
for spec in myspecies:
    gam_spec = pd.read_pickle(f'GAM_species/species_fit_{n_splines}_0.01.pkl')[spec]
    plt.plot(gam_spec['x'], gam_spec['y'], label=spec)
plt.xlabel('cal BP')
plt.ylabel('Abundance (GAM)')
plt.title('GAM of pollen abundances - Basa de la Mora')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small', ncol=2)
plt.tight_layout()
plt.savefig('plots-tests/basa_GAM_abundances_all_species.pdf', dpi=300)
